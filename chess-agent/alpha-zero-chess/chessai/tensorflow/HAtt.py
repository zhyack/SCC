...
class _BaseAttentionMechanism(AttentionMechanism):
  ...
  def __init__(self,
    ...
    """
    self._max_high_layer_num = max_high_layer_num
  ...
  @property
  def values(self):
    return tf.reshape(self._values, [self.batch_size, self._max_high_layer_num*self.alignments_size, self._values.shape[2]])
  ...
  @property
  def batch_size(self):
    return self._batch_size//self._max_high_layer_num
  ...
  def initial_alignments(self, batch_size, dtype):
    ...
    """
    max_time = self._alignments_size*self._max_high_layer_num
...
class HierarchicalLuongAttention(_BaseAttentionMechanism):
  ...
  def __init__(self,
               num_units, 
               max_num_high_layer,
               memory, #[batch_size*max_num_high_layer, num_units]
               higher_layer_scores, #[batch_size*max_num_high_layer]
               memory_sequence_length=None, #[batch_size*max_num_high_layer]
               scale=False,
               probability_fn=None,
               score_mask_value=None,
               dtype=None,
               name="HierarchicalLuongAttention"):
    ...
    super(HierarchicalLuongAttention, self).__init__(
        ...
        memory=memory,
        max_high_layer_num=max_num_high_layer,
        probability_fn=wrapped_probability_fn,
        memory_sequence_length=memory_sequence_length,
        score_mask_value=score_mask_value,
        name=name)
    self._num_units = num_units
    self._max_num_high_layer = max_num_high_layer
    self._high_layer_scores = tf.reshape(higher_layer_scores, [-1, 1]) # [batch_size*max_num_high_layer, 1]
    self._scale = scale
    self._name = name
  ...
  def __call__(self, query, state):
    ...
    Args:
      ...
      state: Tensor of dtype matching `self.values` and shape
        `[batch_size, max_num_high_layer*alignments_size]`
        (`alignments_size` is memory's `max_time`).
    Returns:
      alignments: Tensor of dtype matching `self.values` and shape
        `[batch_size, max_num_high_layer*alignments_size]` (`alignments_size` is memory's `max_time`).
    """
    query = tf.contrib.seq2seq.tile_batch(query, self._max_num_high_layer)
    # [batch_size*max_num_high_layer, alignments_size]
    # state = tf.contrib.seq2seq.tile_batch(state, self._max_num_high_layer)
    with variable_scope.variable_scope(None, "hierarchical_luong_attention", [query]):
      score = _luong_score(query, self._keys, self._scale)
    alignments = self._probability_fn(score, state)*self._high_layer_scores
    alignments = tf.reshape(alignments, [self.batch_size, -1])
...
class HierarchicalBahdanauAttention(_BaseAttentionMechanism):
  ...
  def __init__(self,
               ...
               score_mask_value=None,
               dtype=None,
               name="HierarchicalBahdanauAttention"):
    if probability_fn is None:
      probability_fn = nn_ops.softmax
    if dtype is None:
      dtype = dtypes.float32
  ...
  def __call__(self, query, state):
    ...
    """
    query = tf.contrib.seq2seq.tile_batch(query, self._max_num_high_layer)
    with variable_scope.variable_scope(None, "hierarchical_bahdanau_attention", [query]):
