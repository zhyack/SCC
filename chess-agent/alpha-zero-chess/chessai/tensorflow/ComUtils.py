...
from .HAtt import HierarchicalBahdanauAttention, HierarchicalLuongAttention, AttentionWrapper
...
def modelInitHierarchicalAttentionDecoderCell(size, layers, cell_type, input_dropout, output_dropout, encoder_outputs, encoder_outputs_lengths, max_num_high_layer, high_layer_scores, att_type='LUONG'):
    cells = []
    for _ in range(layers):
        config_cell = None
        if cell_type in ['LSTM', 'lstm']:
    ...
    attention_mechanism = None
    if att_type=='LUONG':
        attention_mechanism = HierarchicalLuongAttention(size, max_num_high_layer, encoder_outputs, high_layer_scores, memory_sequence_length=encoder_outputs_lengths, scale=True)
    elif att_type=='BAHDANAU':
        attention_mechanism = HierarchicalBahdanauAttention(size, max_num_high_layer, encoder_outputs, high_layer_scores, memory_sequence_length=encoder_outputs_lengths, normalize=True)
    else:
        raise Exception('Unknown attention type.')

    op_cell = AttentionWrapper(op_cell, attention_mechanism, attention_layer_size=size, output_attention=True)
...
def modelRunDecoderForBSInfer(cell, inputs, word_embedding_matrix, beam_width, id_end, max_len, initial_state, output_projection_layer):
    decoder=tf.contrib.seq2seq.BeamSearchDecoder(cell, word_embedding_matrix, inputs, id_end, initial_state=initial_state, beam_width=beam_width, output_layer=output_projection_layer, length_penalty_weight=0.0)
    outputs, states, sequence_lengths = tf.contrib.seq2seq.dynamic_decode(decoder, impute_finished=False, maximum_iterations=max_len*2)
